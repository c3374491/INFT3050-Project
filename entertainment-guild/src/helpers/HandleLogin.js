...
const processLogin = async (username, password, token) => {
	...
	if (userData?.list?.length > 0) {
		...
		if (await sha256(user.Salt + password) === user.HashPW) {
            ...
            // Fetch the CustomerID from the TO table
            // This request retrieves the CustomerID associated with the user email from the TO table.
            const email = user.Email; 
            const toUrl = `http://localhost:8080/api/v1/db/data/v1/inft3050/TO?where=(Email,eq,${email})`;
            const toData = await fetchData(toUrl, token);
            const customerID = toData?.list?.[0]?.CustomerID;
            ...
            if (customerID) {
                ...
                console.error("CustomerID not found in TO table for the provided email.");
                return {
